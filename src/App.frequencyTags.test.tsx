...
describe('Frequency Library tag controls', () => {
  ...
  it('lets a visible tag filter the library through the existing search and clear again', () => {
    ...
    expect((screen.getByRole('searchbox') as HTMLInputElement).value).toBe('low tone')
    expect(results.getByText('Deep Ground')).toBeTruthy()
    expect(results.getByText('Ground')).toBeTruthy()
    expect(results.queryByText('Open')).toBeNull()

    fireEvent.click(screen.getByRole('button', { name: 'Clear frequency search' }))
    expect((screen.getByRole('searchbox') as HTMLInputElement).value).toBe('')
    expect(results.getByText('Open')).toBeTruthy()
  })
})